...
class GPPNN(nn.Module):
    def __init__(self,
        ...
        self.lr_blocks = nn.ModuleList([LRBlock(hyper_channels, n_feat) for i in range(n_layer)])
        self.pan_blocks = nn.ModuleList([PANBlock(hyper_channels, multi_channels, n_feat, 1) for i in range(n_layer)])
