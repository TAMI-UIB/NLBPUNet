...
class pan_unfolding(nn.Module):
    def __init__(self, hyper_channels=4, mid_channels=64, T=4, **kwargs):
        super().__init__()
        ...
        self.conv_u = nn.ModuleList([nn.Sequential(*[                         
            nn.Conv2d(hyper_channels*(i+1), mid_channels, kSize, padding=(kSize - 1) // 2, stride=1),
            nn.Conv2d(mid_channels, hyper_channels, kSize, padding=(kSize - 1) // 2, stride=1)
        ]) for i in range(T)])
        ...
        self.conv_up = Conv_up(hyper_channels, G0, self.up_factor)
        self.conv_down = Conv_down(hyper_channels, G0, self.up_factor)

        self.rm1 = att_spatial(res_num=3)

        self.NLBlock = blockNL(hyper_channels, 15)
    ...
    def forward(self, **kwargs):
        ...
        for i in range(len(self.conv_u)):
            ...
            decode_v = torch.cat([rm1_s2_0, rm1_s2_1, rm1_s2_2, rm1_s2_3], 1)  # B 4 256 256
